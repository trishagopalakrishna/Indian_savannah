...
As part of the larger pan-India study on explaining the forest-savannah mosaic landscapes in India and its implications for forest restoration, here I process domestic livestock data to estimate the effect of herbivory pressure on Indian savannahs. 
In this document, I evaluate two livestock datasets- Global Gridded Livestock of the World (GLW3) and that of the Indian Livestock Census and process it to estiamte dry matter intake of grass, an indicator of herbivory pressure. 

## Brief description of datasets

### Indian Livestock Census
The Department of Animal Husbandry and Dairying, Ministry of Fisheries, Animal Husbandry and Dairying, Government of India has been completing livestock census since 1919 (as part of the British Colonial Government). 20 livestock censuses have been compelted thus far, with complete results available for the 19th census that was completed in 2012. As of Mar 2021, provisional results only are available for the latest 20th livestock census and hence was not evaluated. 

Indian livestock census is takes place by household per district in every Indian state for cattle,buffalo, goat, sheep, horses & ponies, camels, mithun & yak, pigs, poultry and other animals. For each livestock type, count is presented for different age groups of the livestock type, by sex and by milking condition. 

This data is not spatial and does not include information about the form of livestock raising i.e. industrialized factory farm livestock or free roaming livestock. 

### Global Gridded Livestock of the World (GLW3)
GLW3, the latest livestock data from the UN FAO provides global population densities for cattle, buffalo, horse, sheep, goats, pig, chickens and ducks in each land pixel at spatial resolution of 0.0833 decimal degrees (~10x10km at Equator). The data has been generated using two methods- Dasymetric and Areal Weighted (AW). In the first method, population densities are assigned to different pixels within the census polygon (district for India) using Random Forest modelling approaches. Contrastingly, in AW method, the livestock count is spread evenly across the census polygon with each pixel representing the average number of animals per sqkm of suitable land. AW method is free of the influence of spatial predictor variables and recommended when circularity is important and not spatial resolution of the data. Suitable land in each pixel was calcuated as the land availabel after excluding permanent vaeter bodies/ice snow, areas with human populations> 10000 (Asia & Africa) people per sq km and excluding protected areas (Gilbert et al., 2018).

This dataset is available spatially and uses the 19th Indian Livestock Census. However, unlike the Indian livestock census tabular data, this dataset does not provide information by sex, age group and/or milking condition. Similar to tabular livestock census data, it does not provide information about industrialized vs free roaming livestock. This dataset does not have information about camels and mithun & yak. 

No poultry and pig data was extracted from either datasets. 
...
19th Indian Livestock Census data was scraped from the website dashboard using Python and accordingly formatted to produce consistent tabular data for each livestock type, by age group, sex and milking condition. 

All relevant GIS GLW3 data was downloaded. 
...
#####################Further data cleaning and EDA, plots (separate pdfs)
...
In both datasets, all data in union territories was consumed to be part of the state they are present in. Andaman & Nicobar and Lakshadweep islands were excluded from the analyses. 

At this stage the GLW3 has more advantages than the Indian livestock census data mainly because it is spatial and the population density has been calculated considering suitable land (albiet with some assumption like exclusion of protected areas, which might not be suitable to the larger pan-India study).

## Dry matter intake estimation
I think of herbivory pressure from domestic livestock as total amount of grass eaten/needed from the savannah  per sqkm per year. Amount of grass eaten/needed is the dry matter (DM) intake for each livestock type (kg DM/year). It depends on a variety of variables such as body weight, gut type, various digestion related variables specific to each livestock type, sex and age group. Hence, it is complicated to determine accurately and only estimates can be calculated. 

I compared to sets of equations to estimate dry matter intake from both datasets, which again has its pros and cons. Set 1 of DM equations is from the IPCC Guidelines to estimate emissions from the livestock sector (See chapter 10 available at https://www.ipcc-nggip.iges.or.jp/public/2019rf/vol4.html) suggested by Rosa Maria Roman-Cuesta on the labgroup email listserv. Set 2 of DM equations is from Wolf et al., 2013 suggested by Liza Leroux on the labgroup email listserv. 

The IPCC set of equations are very complicated and use fine detailed data, the kind of data from the Indian livestock census. I chose to use the feed intake estimate equations in simplified Tier 2 method. The IPCC set of equations is only available for cattle (by different age groups and milking condition), which is a disadvantage of this set of equations. 

From Wolf et al., 2013, I used equation 2 i.e. for metabolic demand from Table 1. This is a general equation for any herbivore that the authors have derived as part of this manuscript. This is the advantage of this equation i.e. it can be used for all herbivore types.

I extracted body weight (BW) data from Table 1 Singh et al., 2005. This is a great paper because it provides avergae body weights of all livestock types, by sex and age groups (similar mostly) as described in the Indian Livestock Census data.

### Dry matter intake from GLW3

Since GLW3 data is not available by sex, age group and milking condition, I averaged the body weight of adults for each livestock type from Sing et al. (see comments in code chunk for how Sing et al., groups (sex and age group) were aggregated per livestock type and averaged). I could not apply the IPCC equations because of lack of data (mentioned above)
...
I assumed that each livestock eats 365 days per year. 
...
This resulted in GIS data for each livestock type, of total DM in Mt/ per year.
...
```
### Dry matter intake from Indian Livestock Census (tabular)

Since the Indian Livestock Census data is fine enough (sex, age group, milking condition), at least for cattle I was able to use the IPCC equations. For the remaining livestock types, I used Wolf et al., 2013 but with BW specific to age group and sex. 
...
##1. Cattle
cattle1<-read.csv("C://Users//Trisha_Gopalakrishna//OneDrive - Nexus365//Paper2//Output//Domestic_Livestock_Processed//Archive//state_cattle.csv")
head(cattle1)
names(cattle1)
...
##2. Buffalo
buffalo1<-read.csv("C://Users//Trisha_Gopalakrishna//OneDrive - Nexus365//Paper2//Output//Domestic_Livestock_Processed//Archive//state_buffalo.csv")
head(buffalo1)
names(buffalo1)
...
##3. Sheep
sheep1<-read.csv("C://Users//Trisha_Gopalakrishna//OneDrive - Nexus365//Paper2//Output//Domestic_Livestock_Processed//Archive//state_sheep.csv")
head(sheep1)
names(sheep1)
unique(sheep1$name)
...
##4. Goat
goat1<-read.csv("C://Users//Trisha_Gopalakrishna//OneDrive - Nexus365//Paper2//Output//Domestic_Livestock_Processed//Archive//state_goat.csv")
head(goat1)
names(goat1)
unique(goat1$name)
...
##5. Camel
camel1<-read.csv("C://Users//Trisha_Gopalakrishna//OneDrive - Nexus365//Paper2//Output//Domestic_Livestock_Processed//Archive//state_camel.csv")
head(camel1)
names(camel1)
unique(camel1$name)
...
##6. Horses
horse1<-read.csv("C://Users//Trisha_Gopalakrishna//OneDrive - Nexus365//Paper2//Output//Domestic_Livestock_Processed//Archive//state_horses.csv")
head(horse1)
names(horse1)
unique(horse1$name)
...
##7. Yak
yak1<-read.csv("C://Users//Trisha_Gopalakrishna//OneDrive - Nexus365//Paper2//Output//Domestic_Livestock_Processed//Archive//state_mithun.csv")
head(yak1)
names(yak1)
unique(yak1$name)
unique(yak1$Type)
...
## Comparison of dry matter intake from both datasets, using both equation set
This comparison is confusing as I am trying to compare 2 datasets and two sets of equations.
...
cattle1<-raster("C://Users//Trisha_Gopalakrishna//OneDrive - Nexus365//Paper2//Output//Domestic_Livestock_Processed//Archive//GriddedLivestock_Processed//cattle_DM.tif")
cattle1
...
##2. Buffalo
buffalo1<-raster("C://Users//Trisha_Gopalakrishna//OneDrive - Nexus365//Paper2//Output//Domestic_Livestock_Processed//Archive//GriddedLivestock_Processed//buffalo_DM.tif")
buffalo1
...
##3. Sheep
sheep1<-raster("C://Users//Trisha_Gopalakrishna//OneDrive - Nexus365//Paper2//Output//Domestic_Livestock_Processed//Archive//GriddedLivestock_Processed//sheep_DM.tif")
sheep1
...
##4. Goat
goat1<-raster("C://Users//Trisha_Gopalakrishna//OneDrive - Nexus365//Paper2//Output//Domestic_Livestock_Processed//Archive//GriddedLivestock_Processed//goat_DM.tif")
goat1
...
##5. Horse
horse1<-raster("C://Users//Trisha_Gopalakrishna//OneDrive - Nexus365//Paper2//Output//Domestic_Livestock_Processed//Archive//GriddedLivestock_Processed//horse_DM.tif")
horse1
...
Looking at the graphs, it seems like except for cattle, the GLW3 data (with average BW) provides higher total dry matter intake (pink)
than when considering sex and group averages of BW for the Indian census data. However, GLW data is more complete than Indian census data i.e. GLW data has data for all Indian states. 

For cattle, Indian census data with IPCC equations predicts a lot more dry matter intake (by a lot more), with a lot of state variation.

Discrepancy for cattle is clear. It is the IPCC equations! They seem to predict a lot more DM for different age groups of cattle. But for the remaining, it is interesting that average BW of adult male and female together provides more DM than finer data (average BW specific to sex). 

However considering the completness of GW data, I am inclined to use it instead of India census data. 

## Refinement of GLW3 dry matter intake

I received confirmation from authors Gilbert et al of GLW3 that that to calculate density I need to divide each raster which is absolute animal numbers (when summed within census polygon) with other area raster provided for each livestock. Hence, accounting for this, I calculated the total amount of DM/year/ sqkm of suitable area using wolf et al., equations and avearge BW (irrespective of sex or age group) for all livestock type and considering consumption over 365 days.
...
## Determination of proportion of DM that is grass
From Heraro et al., 2013 Fig S1- India has majority MIA- Mixed Irrigated- Arid and SemiArid Tropics and Subtropics,
MIH- Mixed Irrigated-  Humid and Subhumid Tropics systems of livestock distribution systems (shades of blue). Only a bit of the NW i.e.
part of Rajasthan and Punjab is yellow- LGA- Livestock only Arid and SemiArid Tropics and Subtropics. But I am going to assume that it is mostly MIA and MIH.

From Herraro et al., 2013 Table S10, I extracted the % gras sin the diet of different livestock for MIA and MIH systems-
...
livestock_density_grass_DM<-tmap_arrange(cattle_map, buffalo_map, goat_map,sheep_map, horse_map, ncol=3)
livestock_density_grass_DM
